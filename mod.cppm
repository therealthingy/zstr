 ...
 */
export namespace strict_fstream {
...
std::string strerror() {
    ...
    if (strerror_s(buff.data(), buff.size(), err_num) != 0) {
       ...
       !_GNU_SOURCE) ||                                                                                      \
    defined(__MUSL__) || defined(__ANDROID_API__)
...
struct static_method_holder {
    static std::string mode_to_string(std::ios_base::openmode mode) {
        ...
        static const std::ios_base::openmode mode_val_v[n_modes] = {std::ios_base::in,    std::ios_base::out,
                                                                    std::ios_base::app,   std::ios_base::ate,
                                                                    std::ios_base::trunc, std::ios_base::binary};

        static const char *mode_name_v[n_modes] = {"in", "out", "app", "ate", "trunc", "binary"};
        std::string res;
...
export namespace zstr {

const std::size_t default_buff_size = static_cast<std::size_t>(1 << 20);
...
class Exception : public std::ios_base::failure {
  public:
    ...
    Exception(z_stream *zstrm_p, int ret) : std::ios_base::failure(error_to_message(zstrm_p, ret)) {}
};    // class Exception
...
class istreambuf : public std::streambuf {
  public:
    istreambuf(std::streambuf *_sbuf_p, std::size_t _buff_size = default_buff_size, bool _auto_detect = true,
               int _window_bits = 0)
        : sbuf_p(_sbuf_p), in_buff(), in_buff_start(nullptr), in_buff_end(nullptr), out_buff(), zstrm_p(nullptr),
          buff_size(_buff_size), auto_detect(_auto_detect), auto_detect_run(false), is_text(false),
                ...
                if (is_text) {
                    ...
                    if (!zstrm_p)
                        zstrm_p = std::unique_ptr<detail::z_stream_wrapper>(
                            new detail::z_stream_wrapper(true, Z_DEFAULT_COMPRESSION, window_bits));
    ...
    int deflate_loop(int flush) {
        while (true) {
            ...
            std::streamsize sz = sbuf_p->sputn(
                out_buff.get(), reinterpret_cast<decltype(out_buff.get())>(zstrm_p->next_out) - out_buff.get());
...
class istream : public std::istream {
  public:
    istream(std::istream &is, std::size_t _buff_size = default_buff_size, bool _auto_detect = true,
            int _window_bits = 0)
...
class ostream : public std::ostream {
  public:
    ostream(std::ostream &os, std::size_t _buff_size = default_buff_size, int _level = Z_DEFAULT_COMPRESSION,
            int _window_bits = 0)
...
class ofstream : private detail::strict_fstream_holder<strict_fstream::ofstream>, public std::ostream {
  public:
